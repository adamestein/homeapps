...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'IncomeTemplate'
        db.create_table(u'finances_incometemplate', (
            ...
            ('amount', self.gf('djmoney.models.fields.MoneyField')(default_currency='USD', null=True, max_digits=10, decimal_places=2, blank=True)),
            ('arrival_day', self.gf('django.db.models.fields.PositiveSmallIntegerField')(null=True, blank=True)),
        ))
        db.send_create_signal(u'finances', ['IncomeTemplate'])

        # Adding M2M table for field options on 'IncomeTemplate'
        m2m_table_name = db.shorten_name(u'finances_incometemplate_options')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('incometemplate', models.ForeignKey(orm[u'finances.incometemplate'], null=False)),
            ('option', models.ForeignKey(orm[u'finances.option'], null=False))
        ))
        db.create_unique(m2m_table_name, ['incometemplate_id', 'option_id'])

        # Adding unique constraint on 'IncomeTemplate', fields ['name', 'user']
        db.create_unique(u'finances_incometemplate', ['name', 'user_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'IncomeTemplate', fields ['name', 'user']
        db.delete_unique(u'finances_incometemplate', ['name', 'user_id'])

        # Deleting model 'IncomeTemplate'
        db.delete_table(u'finances_incometemplate')

        # Removing M2M table for field options on 'IncomeTemplate'
        db.delete_table(db.shorten_name(u'finances_incometemplate_options'))
